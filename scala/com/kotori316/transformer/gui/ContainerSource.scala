...
import com.kotori316.transformer.network.{PacketHandler, SourceGUIMessage}
...
class ContainerSource(player: EntityPlayer, tile: TileSource) extends Container {
  ...
  if (!tile.getWorld.isRemote) {
    PacketHandler.INSTANCE.sendToPlayer(new SourceGUIMessage(tile), player.asInstanceOf[EntityPlayerMP])
  }
